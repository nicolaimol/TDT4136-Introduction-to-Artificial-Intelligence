...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def decide_next_value(self, game_state, depth):
        ...
        agent = depth % game_state.getNumAgents()
        if agent >= 1:
    ...
    def max_value(self, game_state, depth):
        ...
        for action in actions:
            ...
            action_value = self.decide_next_value(next_state, depth + 1)[1]
            value = max(value, (action, action_value), key=lambda x: x[1])
    ...
    def min_value(self, game_state, depth):
        ...
        for action in actions:
            next_state = game_state.generateSuccessor(agent, action)
            action_value = self.decide_next_value(next_state, depth + 1)[1]
            value = min(value, (action, action_value), key=lambda x: x[1])
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        return self.decide_next_value(gameState, 0, alpha, beta)[0]

    def decide_next_value(self, game_state, depth, alpha, beta):
        if depth == game_state.getNumAgents() * self.depth or game_state.isWin() or game_state.isLose():
            return None, self.evaluationFunction(game_state)
    ...
    def max_value(self, game_state, depth, alpha, beta):
        ...
        for action in actions:
            ...
            action_value = self.decide_next_value(next_game_state, depth + 1, alpha, beta)[1]
            value = max(value, (action, action_value), key=lambda x: x[1])

            if value[1] > beta[1]:
                return value

            alpha = max(alpha, value, key=lambda x: x[1])
    ...
    def min_value(self, game_state, depth, alpha, beta):
        ...
        for action in actions:
            next_game_state = game_state.generateSuccessor(agent, action)
            action_value = self.decide_next_value(next_game_state, depth + 1, alpha, beta)[1]
            value = min(value, (action, action_value), key=lambda x: x[1])
            ...
            beta = min(beta, value, key=lambda x: x[1])
